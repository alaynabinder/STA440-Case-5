...
Both `transportNotify` and age were imputed to rely on the non-missing features in the data as well as the random effects for site. We present `transportNotify` and age as categorical and normally distributed variables respectively and set noninformative priors. Specifiically, for age we use the priors $\beta_{0,\text{age}} \sim \mathcal{N}(0,1000)$, $\tau_{\text{age}}, \tau_{b,\text{age}} \sim \text{Gamma}(0.01,0.01)$. For, the categorical `transportNotify`, we adopt $\text{Dirichlet}(\mathbf{1})$ priors. These priors were chosen to allow for data to inform the imputation. 
...
One challenge in coding the model was ensuring categorical variables has baselines set, which was done to ensure reasonable interpretability of effects. 

The model was implemented in JAGS and estimated using Markov chain Monte Carlo (MCMC). We ran three parallel chains for 20,000 iterations each, discarded an initial burn-in period, and applied thinning to reduce autocorrelation. All components of the model, including the logistic regression parameters and the imputation submodels for age, race, and `transportNotify`, were sampled jointly at each iteration. Missing covariates were treated as unknown parameters and drawn from their posterior predictive distributions during sampling, which allowed uncertainty from missing data to propagate appropriately into the posterior distribution of the outcome model.

## Model evaluation

To examine convergence of our sampler, we examined traceplots (Figures @ref(fig:fit1)), effective sample size (Table @ref(tab:fit2)), Rhat values (Table @ref(tab:fit3)), and autocorrelation function plots (Figure @ref(fig:fit4)) for our parameters. Convergence diagnostics were strong for all parameters except for age, which is likely due to the fact that age was entirely missing for certain hospitals. Since we are imputing these age values each iteration of our sampler, there is a lot of variability and sensitivity in age. Still, the effective sample size for age is around 290, and we felt confident enough in our convergence to interpret results.

We also computed a binned residual plot again to compare it to our frequentist fit and examine model assumptions (Figure @ref(fig:fit5)). We saw improvement in the residual plot in terms of randomness compared to our frequentist model, as there does not appear to be a trend in the residuals or any heteroskedascticity.

## Model results

Table @ref(tab:bayes1) shows that the Bayesian model reaffirms the same core associations observed previously. Figure @ref(fig:bayes2) visualizes these effects on the odds ratio scale. Age shows the most certain effect, with a negative coefficient and very small 95 percent credible interval that lies entirely below one, indicating a decline in the odds of favorable discharge as age increases. African American race, female sex, having thrombectomy, and thrombolysis/TPA complications likewise have posterior odds ratios credibly below one, reinforcing their negative associations. In contrast, the TPA without complications group shows an odds ratio clearly above one, providing strong evidence of improved odds of favorable discharge. The only other treatment related factor with a certain positive effect was patients who were not transported by EMS. The factor corresponding to patients who were transported by EMS but did not have pre-hospital notification did not appear to have a credibly large impact on discharge outcome, given its proximity to 1. 

The time indicators show uncertain but generally positive shifts relative to the baseline period. The middle period overlaps substantially with the final period, but the latter has a higher median effect, indicating a modest trend toward better outcomes as the program matured. Although the credible intervals include values near one, the posterior still provides some evidence that the program was associated with improved discharge results.

The Bayesian results are very similar to those from the frequentist model: almost all effect directions and substantive conclusions agree. The main difference is the narrower posterior credible intervals (compared to the confidence intervals), which is expected because the Bayesian approach incorporates prior structure that stabilizes estimation and reduces uncertainty when the data strongly support a given effect.
...
In conclusion, we used a Bayesian logistic regression model with site-level random effects and integrated imputation to assess how demographic, clinical, and operational variables relate to the probability of favorable discharge to home or rehab. Age and treatment-related factors, particularly the presence or absence of complications, emerged as the strongest contributors to outcome, while patterns involving arrival mode and other patient characteristics were less definitive and may reflect differences in underlying severity rather than differences in care. The time indicators, evaluated relative to the baseline of the first two quarters, suggested modest improvement by the final two quarters, although these estimates were similar to those from the implementation period and remained uncertain. As a result, the evidence for a program-wide effect on outcomes is limited.
...
data.frame(
  ...
  kableExtra::kable(caption = "Posterior Summary Statistics")
```
...
```{r label = fit1, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "Traceplots for all parameters", fig.show = 'hold'}

# ---------- Helper Functions ----------
as_matrix_draws <- function(mcl) {
  do.call(rbind, lapply(mcl, function(x) as.matrix(x)))
}

safe_gelman <- function(mcl){
  tryCatch(
    gelman.diag(mcl, multivariate = FALSE),
    error = function(e) {
      message("gelman.diag error: ", e$message)
      return(NULL)
    }
  )
}

extract_psrf <- function(gel) {
  if(is.null(gel)) return(NULL)
  psrf <- as.data.frame(gel$psrf)
  if(ncol(psrf) >= 1) {
    colnames(psrf) <- make.names(colnames(psrf))
    rhat_col <- colnames(psrf)[1]
    rhat_upper_col <- if(ncol(psrf) >= 2) colnames(psrf)[2] else NA
    rhat_df <- data.frame(
      Parameter = rownames(psrf),
      Rhat = as.numeric(psrf[[rhat_col]]),
      Rhat_upper = if(!is.na(rhat_upper_col)) as.numeric(psrf[[rhat_upper_col]]) else NA,
      stringsAsFactors = FALSE
    )
    return(rhat_df)
  }
...
cat("Chains:", length(samples), 
    " Total draws:", nrow(samples_mat), 
    " Parameters:", n_params, "\n\n")
...
mcmc_long <- bind_rows(lapply(seq_along(samples), function(ci){
  ...
  pivot_longer(df, cols = -c(iter, chain), 
               names_to = "Parameter", 
               values_to = "Value")
}))

# Create traceplots in batches of 12 for readability
params_per_page <- 12
n_pages <- ceiling(n_params / params_per_page)

for(page in 1:n_pages){
  start_idx <- (page - 1) * params_per_page + 1
  end_idx <- min(page * params_per_page, n_params)
  page_params <- param_names[start_idx:end_idx]
  
  trace_df <- mcmc_long %>% filter(Parameter %in% page_params)
  
  p_trace <- ggplot(trace_df, aes(x = iter, y = Value, color = chain)) +
    geom_line(alpha = 0.7, linewidth = 0.3) +
    facet_wrap(~ Parameter, scales = "free_y", ncol = 3) +
    labs(
      title = paste0("Traceplots (Page ", page, " of ", n_pages, ")"),
      x = "Iteration",
      y = "Value",
      color = ""
    ) +
...
```{r label = fit2, echo = FALSE, warning = FALSE, message = FALSE}

# ---------- 2) EFFECTIVE SAMPLE SIZE TABLE ----------
...
cat("\nEffective Sample Size (lowest first):\n")
kableExtra::kable(
  ess_df,
  caption = "Effective sample size for all parameters",
  col.names = c("Parameter", "Effective Sample Size")
)
```

```{r label = fit3, echo = FALSE, warning = FALSE, message = FALSE}

# ---------- 3) R-HAT TABLE ----------
gel <- safe_gelman(samples)
rhat_df <- extract_psrf(gel)

rhat_df <- rhat_df %>%
  mutate(Rhat = round(Rhat, 4),
         Rhat_upper = round(Rhat_upper, 4)) %>%
  arrange(desc(Rhat))
  
cat("\nR-hat Values (highest first):\n")
kableExtra::kable(
  rhat_df,
  caption = "Gelman-Rubin Diagnostic (R-hat) for All Parameters",
  col.names = c("Parameter", "Point Estimate", "Upper CI")
)
```

```{r label = fit4, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "ACF plots for all parameters", fig.show = 'hold'}

# ---------- 4) ACF PLOTS ----------
# Select parameters with lowest ESS for ACF inspection
max_lag <- 30
n_acf_params <- min(24, n_params)
acf_candidates <- ess_df$Parameter[1:n_acf_params]
...
acf_df <- bind_rows(acf_list) %>% filter(Lag > 0)
...
p_acf <- ggplot(acf_df, aes(x = Lag, y = ACF)) +
  ...
  labs(
    title = paste0("Autocorrelation Function"),
    x = "Lag",
    y = "ACF"
  ) +
  theme_minimal(base_size = 10) +
...
```{r label = fit5, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "Bayesian binned residual plot", fig.show = 'hold'}
