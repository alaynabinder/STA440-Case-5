...
=======
>>>>>>> 1286d651b11bf34baf0f50cdab017328ed4394b0
# EDA (MISSINGNESS)
...
```{r}
##### SENSITIVITY ANALYSIS — AGE + EMSvsCar (NO PREHOSPITAL NOTIFY) #####
...
delta_ems <- c(-1, 0, 1)        # flipping EMSvsCar
...
for(a in delta_age){
  for(e in delta_ems){
    ...
    adjusted_dfs <- lapply(completed_fixed, function(df){
      ...
      ##### SAFETY CHECK #####
      # If EMSvsCar collapses to one level, skip this dataset
    ...
    # Remove NULLs from collapsed scenarios
    adjusted_dfs <- Filter(Negate(is.null), adjusted_dfs)
    
    # If ALL datasets collapsed → skip entire scenario
    if(length(adjusted_dfs) == 0){
      scenario_name <- paste0("Age_", a, "_EMS_", e)
...
##### 3. Example: view results #####
sensitivity_results[["Age_0_EMS_0"]]
